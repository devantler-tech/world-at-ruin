...
func TestTokenRoundTrip(t *testing.T) {
	...
	tok, err := MintToken(secret, 7, time.Now().Add(time.Minute))
	if err != nil {
...
func TestTokenRefusals(t *testing.T) {
	...
	valid, err := MintToken(secret, 7, time.Now().Add(time.Minute))
	if err != nil {
		t.Fatalf("MintToken: %v", err)
	}
	expired, err := MintToken(secret, 7, time.Now().Add(-time.Minute))
	if err != nil {
		t.Fatalf("MintToken(expired): %v", err)
	}
	forged, err := MintToken(testSecret(0x5A), 7, time.Now().Add(time.Minute))
	if err != nil {
	...
	forgedAndExpired, err := MintToken(testSecret(0x5A), 7, time.Now().Add(-time.Minute))
	if err != nil {
	...
	cases := []struct {
		...
		{"wrong prefix", "v2" + valid[2:], ErrTokenFormat},
...
func TestShortSecretRefused(t *testing.T) {
	...
	if _, err := NewHMACVerifier(short); err == nil {
		t.Fatal("NewHMACVerifier accepted a short secret")
	}
	if _, err := MintToken(short, 1, time.Now().Add(time.Minute)); err == nil {
