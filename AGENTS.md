...
- **Structure:** `client/` is the Godot 4 project (scenes built in GDScript from engine
  ...
  capability 7, backed by the retained v0.93.1 whole-app reader with full-precision vault serialization
  ([retention evidence](docs/evidence/issue-658-mastery-retention/README.md)). The v0.91.0 baseline
  reads mastery but rounds a maximum quest counter on rewrite. Only a real mastery transition
  ...
  append-only/max merging is unsafe for a consumed bloodstain. The boot and abrupt-process-loss
  regressions pin the runtime wiring, banking and one-time reclaim. **The lock lives in
  ...
  **versioned wire codec** that frames that payload (`server/wire/` — transport-agnostic binary
  ...
  exact Nakama account still exists and is enabled; the strict schema and complete
  identity-address contract are base-anchored before this default-off writer is composed), the
  **player handoff core**
