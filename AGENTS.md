...
- **Structure:** `client/` is the Godot 4 project (scenes built in GDScript from engine
  ...
  **player-state mutation boundary** (`server/playerstate/` — atomically commits one private
  ...
  under the verified account identity, refuses public, malformed or stale observations, keeps every
  shipped schema readable through a ledgered golden, and delegates every conditional write to the
  atomic `playerstate` record-plus-audit boundary; it remains inert until a server caller composes
  it), the
