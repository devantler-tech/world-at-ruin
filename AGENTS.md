...
- **Structure:** `client/` is the Godot 4 project (scenes built in GDScript from engine
  ...
  **Nakama handoff lease store** (`server/nakamalease/` — server-owned, strict-schema objects keyed
  ...
  replacement/claim/release preserve one current attempt under retries, with a durable releasing
  barrier that arbitrates zone claim against external cleanup, without persisting raw user or
  reservation IDs or admission secrets), the durable **handoff allocation coordinator**
  (`server/handoffalloc/` — implements `handoff.Allocator` over the real lease store and an injected
  GameServer-resource boundary; makes allocation durable before returning connection material,
  resolves retries, protects claimed/stale ownership and reclaims staged failures), and
  ...
  cross-platform golden), with the concrete Agones/secret resource adapter, zone claim adapter,
  Nakama RPC registration and broader persistence arriving as later children of the
