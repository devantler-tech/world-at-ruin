...
| Same reservation, newer transport attempt | Adopt an ambiguous older dispatch as the durable owner and observe only its exact attempt, or resolve its unexpired finalized allocation; never dispatch the transient retry. Finalization distinguishes the conditional writer from a same-owner replay, and every reused or replayed allocation is retained on downstream failure so an adopter and the original dispatcher cannot release another caller's handoff; the existing no-show lease owns cleanup. Only an expired older lease is marked `releasing`, deleted by exact UID, and replaced. A new logical operation uses a new reservation. |
