...
| Same reservation, newer transport attempt | Adopt an ambiguous older dispatch as the durable owner and observe only its exact attempt; never dispatch the transient retry. Carry the persisted owner through later handoff cleanup. Otherwise mark the older lease `releasing`, delete only its exact UID, then stage the new attempt. A new logical operation uses a new reservation. |
