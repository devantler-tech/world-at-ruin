...
**Refinement recorded with #818 (2026-09-22):** step 5's definitive unallocated
response is Agones's exact UnAllocated status (no Ready GameServer matched — an
empty pool) or its Contention status, matched by code and message so a transport
limit sharing the code stays ambiguous. It is only ever read from the one
dispatch; observation cannot produce it. Agones v1.60 retries a request whose
GameServer update it saw fail and answers UnAllocated once a retry finds no Ready
GameServer, yet the failed update may have committed. So the answer proves the
allocator completed no allocation for the request, not that no write it sent can
still land. The coordinator therefore settles it in the order the orphan reconciler
already relies on: fence the exact dispatched version as `releasing`, release by
the attempt label (deleting anything that did commit by its own UID), then delete
...
| No-show expiry | For a non-ambiguous unclaimed lease, the supervised reconciler begins release, deletes the exact GameServer with a UID precondition, and retries transient storage or resource-cleanup failures until host cancellation. A dispatched no-match lease remains quarantined past expiry until a late object is released or an allocator-generation fence clears it. |
...
late server-side commit cannot race a second allocation. An empty pool is not
ambiguous and does not quarantine: the allocator's definitive answer releases
the attempt at once (see the #818 refinement), accepting that a write the answer
hid and that lands after the release is briefly a leaked, never-admitted
GameServer until the orphan reconciler's grace elapses. The
Agones SDK sidecar retains its official Kubernetes credential, but the zone
container's token path is shadowed and raw admission material never enters
Kubernetes credentials or Secrets.
