...
| Finalization distinguishes its conditional writer from a same-owner replay; every reused or replayed allocation is retained on downstream handoff failure, so concurrent adopters and the original dispatcher reconcile one winner without any loser tearing it down | `nakamalease.Store` + `handoff.Service` + `handoffalloc.Coordinator` | `TestFinalizeReportsWhetherTheObservedWriterCommitted`, `TestServiceRetryRetainsAReconciledAttemptWhenLaterStageFails`, `TestOverlappingTransportRetriesRetainTheSharedDurableAllocation`, `TestDispatchOwnerRetainsTheWinnerWhenAnAdopterFinalizesFirst` |
