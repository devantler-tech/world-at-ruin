...
| A transport retry adopts the quarantined dispatch owner for observation-only reconciliation, never redispatches, never crosses a release fence, and carries that persisted owner through later cleanup | `handoff.Service` + `handoffalloc.Coordinator` | `TestAllocateAdoptsQuarantinedDispatchWithoutRedispatch`, `TestAllocateRetryCannotAdoptAQuarantineWhoseReleaseHasBegun`, `TestServiceRetryReconcilesTheQuarantinedAttemptWithoutRedispatch`, `TestServiceRetryCleansAReconciledAttemptByItsPersistedOwner` |
