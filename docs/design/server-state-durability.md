...
| Every reused allocation is retained on downstream handoff failure, and concurrent adopters reconcile the same durable winner without either failure tearing it down | `handoff.Service` + `handoffalloc.Coordinator` | `TestServiceRetryRetainsAReconciledAttemptWhenLaterStageFails`, `TestOverlappingTransportRetriesRetainTheSharedDurableAllocation` |
