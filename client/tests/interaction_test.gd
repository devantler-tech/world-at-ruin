...
func _ready() -> void:
	...
	# The integration boot exercises the first-run creator path, so a LIVE
	# character save is player state (no-resets law): back it up and restore it
	# whatever happens.
...
# --- live-save guard (crash-safe backup/restore, as in npc_population_test) --
